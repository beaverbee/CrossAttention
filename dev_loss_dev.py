...
tokenizer = get_tokenizer('uncased_L-12_H-768_A-12/vocab.txt')
...
class data_generator:
    def __init__(self, data, tokenizer, rel2id, num_rels, maxlen, batch_size=6):
        self.data = data
    ...
    def __iter__(self):
        while True:
            ...
            for idx in idxs:
                ...
                tokens = self.tokenizer.tokenize(text)  # 在标准Bert中tokenize是负责在vocal中匹配字符的，只有匹配的字符才能encoder
                if len(tokens) > 100:
                    tokens = tokens[:100]
                text_len = len(tokens)
                ...
                if s2ro_map:
                    ...
                    if len(tokens_batch) == self.batch_size or idx == idxs[-1]:
                        ...
                        yield [tokens_batch, segments_batch, sub_heads_batch, sub_tails_batch, sub_head_batch, sub_tail_batch, obj_heads_batch,
                               obj_tails_batch,text,line['triple_list']], None
                        tokens_batch, segments_batch, sub_heads_batch, sub_tails_batch, sub_head_batch, sub_tail_batch, obj_heads_batch, obj_tails_batch, = [], [], [], [], [], [], [], []
...
train_generator = data_generator(test_data, tokenizer, rel2id, num_rels, maxlen=100, batch_size=1)  # 生成迭代器
print(train_generator.__iter__().__next__()[0][2])
print(train_generator.__iter__().__next__()[0][3])
print(train_generator.__iter__().__next__()[0][-2])
print(train_generator.__iter__().__next__()[0][-1])
