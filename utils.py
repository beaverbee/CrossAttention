...
def partial_match(pred_set, gold_set):
    pred = {(i[0].split(' ')[0] if len(i[0].split(' ')) > 0 else i[0], i[1],
             i[2].split(' ')[0] if len(i[2].split(' ')) > 0 else i[2]) for i in
            pred_set}
...
def att_feature(x):
    seq, score, k_seq = x
    score = K.softmax(score, -2)
    att = score * k_seq
    att = K.sum(att, 1, keepdims=True)
    # seq += att
    temp = K.zeros_like(seq)
    temp = K.cast(temp, dtype='float32')
    att = att + temp
    seq = K.concatenate([seq, att], axis=-1)
    # seq_len = K.int_shape(seq)[-1]
    return seq


def concentrate(x):
    seq, sub_feature = x
    sub_feature = K.expand_dims(sub_feature, axis=1)
    temp = K.zeros_like(seq)
    temp = K.cast(temp, dtype='float32')
    sub_feature = temp + sub_feature
    seq = K.concatenate([seq, sub_feature], -1)
    return seq
